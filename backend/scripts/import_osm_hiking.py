"""
Import global hiking trails from OpenStreetMap into route-db/seed_routes.json —
the country-agnostic sibling of import_osm_cai.py.

    python scripts/import_osm_hiking.py [--max-per-area 3] [--dry-run]
        [--endpoint URL] [--from-file inputs.json] [--area area-id]

Same pipeline as import_osm_cai.py (phase A candidates -> phase B geometry ->
decimate -> Open-Meteo elevation -> sanity gate), but generalized for
non-Italian networks:
  - Phase A drops the `cai_scale` requirement (an Italian-CAI-network-specific
    OSM tag that simply does not exist outside Italy) — any named
    `route=hiking` relation qualifies.
  - Difficulty: `sac_scale` (Swiss/Austrian/Alpine-club convention, also used
    in parts of Norway) taken verbatim if present, else "n.d." — NEVER
    inferred, matching the hard rule of every importer in this repo.
  - Elevation/length sanity ranges are widened for global terrain (sea-level
    coastal trails to high alpine, and longer multi-day treks) — the
    Alps-only 200-4000 m / 1-30 km ranges in import_osm_cai.py would
...
- --from-file <json>: offline, same shape as import_osm_cai.py.
"""
...
#: Aree fuori dalla rete CAI (import_osm_cai.py resta lo script per l'Italia) —
#: bbox (lat_min, lat_max, lon_min, lon_max), più larghe delle aree sci/falesia
#: perché coprono parchi/regioni intere, non un singolo comprensorio.
AREAS: dict[str, tuple[float, float, float, float]] = {
    "area-yosemite-us": (37.50, 38.05, -119.90, -119.15),
    "area-zion-us": (37.10, 37.45, -113.10, -112.75),
    "area-jackson-hole-us": (43.35, 44.05, -111.10, -110.55),
    "area-hakuba-jp": (36.55, 36.85, 137.75, 137.95),
    "area-fuji-jp": (35.25, 35.50, 138.62, 138.85),
    "area-whistler-ca": (49.95, 50.20, -123.20, -122.80),
    "area-banff-ca": (51.05, 51.55, -116.30, -115.30),
    "area-queenstown-nz": (-45.15, -44.85, 168.55, 168.90),
...
#: Ampliati rispetto alle Alpi italiane (import_osm_cai.py: 1-30 km,
#: 200-4000 m): trekking di più giorni (Milford Track, tour del Fuji) e
#: quote da livello del mare (Fiordland, Lofoten) fuori da quel range
#: sarebbero SCARTATI a torto, non perché i dati siano sospetti.
MIN_LEN_M, MAX_LEN_M = 1000.0, 60000.0
MIN_ELE_M, MAX_ELE_M = -50.0, 6000.0
SKIP_ROLES = ("alternat", "excursion", "approach", "variant", "connection")


# ── geometry helpers (identiche a import_osm_cai.py) ────────────────────────
...
def phase_a_query(area_id: str) -> str:
    ...
    return f'[out:json][timeout:25];relation["route"="hiking"]["name"]{bbox};out tags center {PHASE_A_LIMIT};'
...
def candidates_for_area(resp: dict) -> list[dict]:
    """Phase-A relations mapped to candidates, ref-holders first — stesso
    ordinamento di import_osm_cai.py, ma senza il requisito cai_scale."""
    cands = []
    for el in resp.get("elements", []):
        ...
        cands.append({"id": el["id"], "name": name, "ref": ref,
                      "sac_scale": (tags.get("sac_scale") or "").strip(),
...
def build_route(area_id: str, cand: dict, coords: list[tuple[float, float]],
    ...
    return {
        ...
        "diff_scale": "OSM",
        "diff_grade": cand["sac_scale"] or "n.d.",  # mai inventata
        "diff_index": None,
...
def sanity(area_id: str, coords: list[tuple[float, float]],
    ...
    if not (MIN_LEN_M <= length_m <= MAX_LEN_M):
        return f"lunghezza {length_m / 1000:.1f} km fuori range {MIN_LEN_M/1000:.0f}-{MAX_LEN_M/1000:.0f} km"
    for e in eles:
...
# ── input providers (live vs --from-file) — identici a import_osm_cai.py ────
class LiveProvider:
